...
(defn new [buf &opt encode-fn time-fn]
  ...
  (fn [&keys keys]
    ...
    (def currentlvl (index-of *level* levels))
    (if (>= lvl currentlvl)
...
(defn debug [msg &keys keys] (*default* :level :debug :msg msg ;(kvs keys)))
(defn info [msg &keys keys] (*default* :level :info :msg msg ;(kvs keys)))
(defn warn [msg &keys keys] (*default* :level :warn :msg msg ;(kvs keys)))
(defn error [msg &keys keys] (*default* :level :error :msg msg ;(kvs keys)))
(defn fatal [msg &keys keys] (*default* :level :fatal :msg msg ;(kvs keys)))
(defn unknown [msg &keys keys] (*default* :level :unknown :msg msg ;(kvs keys)))

(defn set-default [l] (set *default* l))
(defn set-level [l] (set *level* (index-of l levels)))
